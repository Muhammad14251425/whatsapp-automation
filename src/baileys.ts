...
export class BaileysClass extends EventEmitter {
    ...
    sendMedia = async (number: string, mediaUrl: string, text: string): Promise<any> => {
        try {
            // Check if it's a Google Drive shared link and convert to direct download URL
            const finalUrl = this.getDirectGoogleDriveUrl(mediaUrl);

            const fileDownloaded = await utils.generalDownload(finalUrl);
     ...
     */
    getDirectGoogleDriveUrl = (url: string): string => {
        const match = url.match(/https?:\/\/drive\.google\.com\/file\/d\/([^\/]+)\//);
        if (match && match[1]) {
            return `https://drive.google.com/uc?export=download&id=${match[1]}`;
        }
        return url; // Return original URL if it's not a Google Drive file link
    }
